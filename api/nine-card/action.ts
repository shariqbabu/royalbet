    ...
    switch (type) {
      ...
      case 'call': {
        ...
        await internalWalletTransaction({
          ...
          idempotencyKey: `9card_call_${tableId}_${table.round}_${uid}_${callTs}`,
        });

        // Turn-check + state update ek transaction mein — pehle bahar tha,
        // double-tap pe dono requests deduct kar leti thin (turn race)
        try {
          await db.runTransaction(async (tx) => {
            const fresh = await tx.get(tableRef);
            if (!fresh.exists) throw new Error('Table not found');
            const cur = fresh.data() as NineCardTable;

            if (cur.currentTurn !== uid)  throw new Error('Not your turn');
            if (cur.status !== 'playing') throw new Error('Game not active');
            const player = cur.players[uid];
            if (!player) throw new Error('Player not found');

            const activePlayers = cur.playerOrder.filter(id => cur.players[id]?.status !== 'packed');
            const myIdx         = activePlayers.indexOf(uid);
            const nextUid       = activePlayers[(myIdx + 1) % activePlayers.length];

            const updatedPlayers = { ...cur.players };
            updatedPlayers[uid] = {
              ...player,
            ...
            tx.update(tableRef, {
              players:         updatedPlayers,
              pot:             FieldValue.increment(callAmt),
              currentTurn:     nextUid,
              lastRaiseBy:     null,
              lastRaiseAmount: 0,
              updatedAt:       FieldValue.serverTimestamp(),
          ...
          // Deduct ho chuka lekin move apply nahi hua — refund karo
          await internalWalletTransaction({
            action: 'ADD', uid, amount: callAmt,
            type: 'REFUND', game: 'NineCard', balanceType: 'depositBalance',
            description: `9 Card call refund — ${txErr.message}`,
            idempotencyKey: `9card_call_refund_${tableId}_${uid}_${callTs}`,
          }).catch(e => console.error('CRITICAL: call refund failed', e));
      ...
      case 'raise': {
        ...
        await internalWalletTransaction({
          ...
          idempotencyKey: `9card_raise_${tableId}_${table.round}_${uid}_${raiseTs}`,
        });

        // Turn-check + update ek transaction mein — call jaisi hi race thi
        try {
          await db.runTransaction(async (tx) => {
            const fresh = await tx.get(tableRef);
            if (!fresh.exists) throw new Error('Table not found');
            ...
            if (cur.currentTurn !== uid)  throw new Error('Not your turn');
            if (cur.status !== 'playing') throw new Error('Game not active');
            const curPlayer = cur.players[uid];
            if (!curPlayer) throw new Error('Player not found');
            ...
            updatedPlayers[uid] = {
              ...curPlayer,
              currentBet:    (curPlayer.currentBet || 0) + raiseAmount,
              totalBet:      (curPlayer.totalBet   || 0) + raiseAmount,
              isMyTurn:      false,
              turnStartedAt: null,
              autoCallAt:    null,
            };
            ...
            tx.update(tableRef, {
              players:           updatedPlayers,
              pot:               FieldValue.increment(raiseAmount),
          ...
          await internalWalletTransaction({
            action: 'ADD', uid, amount: raiseAmount,
            type: 'REFUND', game: 'NineCard', balanceType: 'depositBalance',
            description: `9 Card raise refund — ${txErr.message}`,
            idempotencyKey: `9card_raise_refund_${tableId}_${uid}_${raiseTs}`,
          }).catch(e => console.error('CRITICAL: raise refund failed', e));
          return res.status(400).json({ error: txErr.message });
        }
